...
function minimalProduct(overrides: Record<string, unknown> = {}): Record<string, unknown> {
  ...
  const base = {
    ...
    normalizedTitle: 'test product title here',
...
describe('validateProductForIngest', () => {
  ...
  it('enforces reviews, angles, and suppliers', () => {
    ...
    const badSource = validateProductForIngest(
      minimalProduct({
        suppliers: [{ source: 'manual', platform: 'Shopify', monthlyTraffic: 10_000 }],
      }),
      'US',
    );
    expect(badSource.some((r) => r.includes('supplier[0] must use apify_store_leads'))).toBe(true);
