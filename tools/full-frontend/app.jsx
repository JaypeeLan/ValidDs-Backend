...
function App() {
  ...
  async function loadTikTokLiveDiscover() {
    ...
    try {
      ...
      setStatus(checked === 0
        ? "No creator handles found — add products with creator handles first"
        : `${liveCount} of ${checked} creator${checked !== 1 ? "s" : ""} live`);
    } catch (err) {
      setTiktokLives([]);
  ...
  useEffect(() => {
    if (page === "TikTok Live" && token) loadTikTokLiveDiscover();
  }, [page]);
  useEffect(() => {
...
function TikTokLivePage({
  ...
  return (
    <div className="page-shell">
      <SectionHeader
        title="TikTok Live"
        subtitle="Live streams from your product creators — updated on every visit. Search extra handles below."
      />
      <div className="filter-panel">
        <div className="filter-row wide">
          <input
            placeholder="Search extra handles (e.g. charlidamelio, khaby.lame) — leave blank to refresh discovery"
            value={handles}
            onChange={(e) => setHandles(e.target.value)}
            onKeyDown={(e) => { if (e.key === "Enter") submit(); }}
          />
          <button className="primary" onClick={submit} disabled={loading}>
            {loading ? "Checking…" : handles.trim() ? "Check handles" : "Refresh"}
          </button>
        </div>
        <p className="dim small">Auto-discovers handles from your tracked products. Search up to 10 extra handles, @ prefix optional.</p>
      </div>
      <ErrorBanner message={error} />
      ...
      <div className="grid shopify-grid" aria-busy={loading}>
        {loading ? (
          ...
          <EmptyState text="No creator live streams found right now. Check back later or search specific handles above." />
        ) : (
          toArray(lives).map((live, idx) => <SCLiveCard key={live?.handle || idx} live={live} />)
        )}
      </div>
    </div>
  );
    ...
    <article className="card shopify-card sc-live-card">
      ...
      <div className="card-body">
        ...
        <div className="actions" style={{ marginTop: "0.5rem" }}>
          <a className="primary" style={{ padding: "0.4rem 0.9rem", fontSize: "0.82rem", borderRadius: "var(--radius-sm)", background: "var(--primary)", color: "#fff", textDecoration: "none", fontWeight: 600 }}
            href={watchUrl} target="_blank" rel="noopener noreferrer">
            Watch live ↗
          </a>
          {streamUrl ? (
            <a className="link-btn" href={streamUrl} target="_blank" rel="noopener noreferrer">Stream URL ↗</a>
          ) : null}
        </div>
      </div>
    </article>
