...
Shopify hits **`GET /api/v1/stores/shopify/callback`** on the **backend**. The backend then **redirects the browser** to:

`{FRONTEND_URL}/stores/shopify/callback?status=success&shop=...`  
or  
`?status=error&code=...&message=...`

**Your SPA route** `/stores/shopify/callback` should:

1. Read `status`, `shop`, `code`, `message` from the query string.
2. Show success or error toast.
3. **`history.replaceState`** (or router replace) to strip query params.
4. Call **`GET /stores/shopify/status`** again to refresh UI.
